...
type MRTEntriesQuerier interface {
	...
	QueryMRTEntriesByOriginASN(ctx context.Context, originASN uint32) (Lister, error)

	// QueryMRTEntriesByASSegments returns entries whose AS path contains
	// all of the given AS numbers (in any order).
	QueryMRTEntriesByASSegments(ctx context.Context, asSegments []uint32) (Lister, error)
	...
	QueryMRTEntriesByNeighborASN(ctx context.Context, neighborASN uint32) (Lister, error)

	// QueryMRTEntriesByIP returns entries whose prefix contains the given
	// IP address (i.e. prefix >> targetIP).
	QueryMRTEntriesByIP(ctx context.Context, targetIP net.IP) (Lister, error)

	// QueryMRTEntriesByCIDR returns entries whose prefix encompasses or
	// matches the given CIDR (i.e. prefix >>= targetCIDR).
	QueryMRTEntriesByCIDR(ctx context.Context, targetCIDR net.IPNet) (Lister, error)
}
