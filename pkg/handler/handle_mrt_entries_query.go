...
func (h *MRTEntriesQueryHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if originASN := q.Get(QueryKeyOriginASN); originASN != "" {
		...
		lister_, err = h.Querier.QueryMRTEntriesByOriginASN(ctx, uint32(asn))
		...
		lister_, err = h.Querier.QueryMRTEntriesByASSegments(ctx, segments)
		...
		lister_, err = h.Querier.QueryMRTEntriesByNeighborASN(ctx, uint32(asn))
		...
		lister_, err = h.Querier.QueryMRTEntriesByIP(ctx, parsedIP)
		...
		lister_, err = h.Querier.QueryMRTEntriesByCIDR(ctx, *parsedCIDR)
	} else {
